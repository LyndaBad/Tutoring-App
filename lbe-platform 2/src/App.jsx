...
import { supabase, isSupabaseEnabled, buildUserFromSession } from './lib/supabase.js';
...
function Login({go,setUser,bp}){
  ...
  const handle=async()=>{
    ...
    if(isSupabaseEnabled){
      ...
      if(data?.user){
        const u=await buildUserFromSession(data.user);
        if(u){setUser(u);go(u.role==="admin"?"admin":u.role==="tutor"?"tutor-dash":u.role==="parent"?"parent":"dashboard");sLoading(false);return;}
      }
      // If error is not just wrong credentials, show it
      if(error&&!error.message.includes("Invalid login credentials")){sErr(error.message);sLoading(false);return;}
    }
    // Fall back to mock users (demo mode)
    const u=USERS.find(u=>u.email===email&&u.pw===pw);
    if(!u){sErr("Incorrect email or password.");sLoading(false);return;}
  ...
  return(
    <div style={{paddingTop:70,minHeight:"90vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
      <div style={{background:T.n2,border:`1px solid ${T.rl}`,borderRadius:20,padding:"3rem",width:"100%",maxWidth:440,boxShadow:T.s2}}>
        ...
        <p style={{fontSize:".75rem",color:T.ash,marginBottom:"2rem"}}>{isSupabaseEnabled?"Sign in with your account below.":"Demo: student@demo.com / demo · parent@demo.com / demo"}<br/>{!isSupabaseEnabled&&"tutor@demo.com / demo · admin@lbe.com / admin"}</p>
        <Inp label="Email" val={email} onChange={e=>sEmail(e.target.value)} type="email" ph="your@email.com"/>
        <Inp label="Password" val={pw} onChange={e=>sPw(e.target.value)} type="password" ph="••••••••" onKeyDown={e=>{if(e.key==="Enter")handle();}}/>
        {err&&<p style={{fontSize:".78rem",color:T.rd,marginBottom:"1rem"}}>{err}</p>}
        <Btn ch={loading?"Signing in…":"Sign In"} v="gold" sz="lg" sx={{width:"100%",justifyContent:"center"}} onClick={handle} dis={loading}/>
...
function Signup({go,setUser}){
  ...
  const[err,sErr]=useState("");const[loading,sLoading]=useState(false);
  const handle=async()=>{
    if(!f.name||!f.email||!f.pw){sErr("Please fill in all fields.");return;}
    sErr("");sLoading(true);
    // Try Supabase if configured
    if(isSupabaseEnabled){
      const{data,error}=await supabase.auth.signUp({
        email:f.email,password:f.pw,
        options:{data:{full_name:f.name,role:f.role}},
      });
      if(error){sErr(error.message);sLoading(false);return;}
      if(data.user){
        // Upsert profile (trigger may have already created it)
        await supabase.from("profiles").upsert({id:data.user.id,email:f.email,full_name:f.name,role:f.role},{onConflict:"id"});
        if(f.role==="tutor"){
          await supabase.from("tutor_profiles").insert({user_id:data.user.id,status:"pending"}).select().maybeSingle();
        }
      }
      const nu={id:data.user?.id||uid(),email:f.email,name:f.name,role:f.role,av:f.name.split(" ").map(w=>w[0]).join("").toUpperCase().slice(0,2),enrollments:[]};
      setUser(nu);go(f.role==="tutor"?"tutor-dash":f.role==="parent"?"parent":"courses");
      sLoading(false);return;
    }
    // Demo mode fallback
  ...
  return(
    <div style={{paddingTop:70,minHeight:"90vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
      <div style={{background:T.n2,border:`1px solid ${T.rl}`,borderRadius:20,padding:"3rem",width:"100%",maxWidth:440}}>
        ...
        <Inp label="Email" val={f.email} onChange={e=>sF({...f,email:e.target.value})} type="email" ph="your@email.com"/>
        <Inp label="Password" val={f.pw} onChange={e=>sF({...f,pw:e.target.value})} type="password" ph="Choose a password (min 6 chars)"/>
        ...
        {err&&<p style={{fontSize:".78rem",color:T.rd,marginBottom:"1rem"}}>{err}</p>}
        <Btn ch={loading?"Creating account…":"Create Account"} v="gold" sz="lg" sx={{width:"100%",justifyContent:"center"}} onClick={handle} dis={loading}/>
...
function BookModal({enrollId,user,setUser,onClose}){
  ...
  const confirm=async()=>{
    ...
    // Persist to Supabase if connected and real user (no pw field = real user)
    if(isSupabaseEnabled&&user.id&&!user.pw){
      await supabase.from("bookings").insert({
        enrollment_id:enrollId,student_id:user.id,
        scheduled_date:selD,scheduled_time:selT,
        status:"scheduled",duration_mins:60,timezone:"Europe/London",
      });
      await supabase.from("enrollments").update({credits_used:enr.used+1}).eq("id",enrollId);
    }
    sDone(true);
  };
  return(
...
function StudentDash({user,setUser,go,bp}){
  ...
  // Refresh real data from Supabase if connected and this is a real (non-demo) user
  useEffect(()=>{
    if(!isSupabaseEnabled||!user?.id||user?.pw)return;
    supabase.from("enrollments").select(`
      id,course_id,package_type,credits_total,credits_used,enrolled_at,
      bookings(id,scheduled_date,scheduled_time,status,zoom_link,zoom_meeting_id),
      assessment_results(id,score,max_score,grade,completed_at,visible_to_student,assessments(title,type))
    `).eq("student_id",user.id).eq("status","active").then(({data})=>{
      if(data?.length){
        const enrs=data.map(e=>({
          id:e.id,courseId:e.course_id,pkg:e.package_type,
          credits:e.credits_total,used:e.credits_used,enrolledAt:e.enrolled_at,
          completedLessons:[],
          bookings:(e.bookings||[]).map(b=>({id:b.id,date:b.scheduled_date,time:b.scheduled_time,status:b.status,zoom:b.zoom_link||"",meetingId:b.zoom_meeting_id||"",tutor:"Lynda Badmus",ln:0})),
          assessments:(e.assessment_results||[]).map(r=>({id:r.id,title:r.assessments?.title||"Assessment",type:r.assessments?.type||"topic_check",score:r.score,max:r.max_score,done:!!r.completed_at,notes:"",date:r.completed_at,strengths:[],work:[]})),
        }));
        setUser(prev=>({...prev,enrollments:enrs}));
      }
    });
...
export default function App(){
    ...
    if(isSupabaseEnabled){
      ...
      supabase.auth.getSession().then(async({data:{session}})=>{
        if(session?.user){
          ...
          // No Supabase session — try localStorage for demo users
          try{const s=localStorage.getItem("lbe-v3");if(s)setUser(JSON.parse(s));}catch{}
        }
      });
      ...
      const{data:{subscription}}=supabase.auth.onAuthStateChange(async(event,session)=>{
        ...
        if(event==="SIGNED_OUT"){setUser(null);localStorage.removeItem("lbe-v3");}
      });
  ...
  // Persist demo users to localStorage (real users handled by Supabase session)
  useEffect(()=>{
    if(!user)return;
    if(user.pw){// demo user — save to localStorage
      try{localStorage.setItem("lbe-v3",JSON.stringify(user));}catch{}
    }
  },[user]);

  const go=useCallback(p=>{setPg(p);setSideOpen(false);requestAnimationFrame(()=>window.scrollTo({top:0,behavior:"smooth"}));},[]);
  const logout=useCallback(async()=>{
    if(isSupabaseEnabled&&user&&!user.pw){
      await supabase.auth.signOut();
    }
    localStorage.removeItem("lbe-v3");
    setUser(null);
    go("home");
  },[go,user]);
